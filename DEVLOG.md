...
This project seems to lend itself fairly well to object-oriented programming, with many classes sharing similar behaviours, such as processing blocks of samples and requiring the sample rate to be updated. However, I am not convinced this is the best approach rather than a convenient approach. So far, I am avoiding multiple inheritance when I can, to avoid overriding virtual functions, even though I haven't measured the performance cost yet to know if this is really an issue. The reasoning that this is likely worthwhile is that these functions would be called many times per second. For example, at a sample rate of 44.1kHz and with a block size of 1024 samples, `processBlock()` would be called 43.07 times per second. At 96kHz and 8 samples per block, it would be called 12000 times per second. I am giving thought to data-oriented design, having watched [a talk by Mike Acton](https://www.youtube.com/watch?v=rX0ItVEVjHc), and to how this might apply to this project.
...
- The JUCE library uses a lot of multiple inheritance and involves overriding virtual functions, which would be good to avoid for performance (I will test this eventually).
